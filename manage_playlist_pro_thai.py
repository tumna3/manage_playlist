...
def resource_path(relative_path):
    """ หา Path ของไฟล์ที่ถูก Pack รวมไว้ใน .exe หรือรันปกติ """
    try:
        # PyInstaller สร้างโฟลเดอร์ชั่วคราวเก็บไฟล์ไว้ที่ _MEIPASS
        base_path = sys._MEIPASS
    except Exception:
        base_path = os.path.abspath(".")
    return os.path.join(base_path, relative_path)

# ─── ฟังก์ชันโหลดฟอนต์เข้า Memory (Windows Only) ──────────────────
def load_custom_font(font_filename):
    font_path = resource_path(font_filename)
    if os.path.exists(font_path):
        # 0x10 คือ FR_PRIVATE (โหลดมาใช้เฉพาะโปรแกรมนี้เท่านั้น)
        ctypes.windll.gdi32.AddFontResourceExW(font_path, 0x10, 0)
    else:
        print(f"Warning: Font file {font_filename} not found.")

# สั่งโหลดฟอนต์ก่อนเริ่มสร้าง UI
# ** สำคัญ: ชื่อไฟล์ต้องตรงกับในโฟลเดอร์ **
# โหลดให้ครบทั้ง 4 ไฟล์ (สมมติชื่อไฟล์ตามมาตรฐาน)
load_custom_font("THSarabun.ttf")          # ตัวธรรมดา
load_custom_font("THSarabun_Bold.ttf")     # ตัวหนา
load_custom_font("THSarabun_Italic.ttf")   # ตัวเอียง
load_custom_font("THSarabun_BoldItalic.ttf")       # ตัวหนาเอียง
...
# เรียกใช้ก่อนสร้าง root = tk.Tk()
load_custom_font("THSarabun.ttf")

FONT_TITLE   = ("THSarabun_Bold.ttf", 18, "bold")
FONT_HEADER  = ("THSarabun_Bold.ttf", 14, "bold")
FONT_NORMAL  = ("THSarabun.ttf", 10)
FONT_SMALL   = ("THSarabun.ttf", 10)
FONT_MONO    = ("THSarabun.ttf", 9)
FONT_LABEL   = ("THSarabun.ttf", 10)
# ─── PLAYLIST PARSER ─────────────────────────────────────────────────────────
...
def make_btn(parent, text, command, color=None, icon="", width=None, tooltip=None):
    ...
    lbl = tk.Label(inner, text=label_text, background=bg,
                   foreground="#000000", font=FONT_SMALL, cursor="hand2")  # แก้เป็นสีดำ
    if width:
...
def make_icon_btn(parent, text, command, size=28, bg=None, fg=None):
    ...
    bg = bg or C["card"]
    fg = fg or "#000000"  # แก้เป็นสีดำ
    btn = tk.Label(parent, text=text, background=bg, foreground=fg,
                   font=("TH Sarabun New", 10), width=2, cursor="hand2",
...
class VLCPlayer:
    ...
    def attach(self):
        ...
        try:
            ...
            plat   = sys.platform
            if plat.startswith("win"): self.player.set_hwnd(win_id)
            elif plat == "darwin":     self.player.set_nsobject(win_id)
    ...
    def pause_resume(self):
        if not self._ready or not self.player: return
        self.player.pause()
    def stop(self):
        if not self._ready or not self.player: return
        self.player.stop()
    def set_volume(self, vol: int):
        ...
        if self._ready and self.player:
            self.player.audio_set_volume(self._volume)
    def is_playing(self):
        if not self._ready or not self.player: return False
...
class PlaylistManagerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="📂 Playlist Manager Pro  by TUM NA...แจกฟรี", font=FONT_TITLE, background=C["bg2"], foreground=C["accent"]).pack(side="left", padx=20, pady=10)
        tk.Label(hdr, text="v1.0  |  M3U & W3U", font=FONT_SMALL, background=C["bg2"], foreground=C["text_muted"]).pack(side="left", pady=10)
        ...
        make_btn(btn_row, "นำเข้าไฟล์เพลยลิสต์",  self.import_playlist, icon="📂", color=C["accent"], tooltip="Open a new .m3u or .w3u playlist").pack(side="left", padx=4)
        make_btn(btn_row, "นำเข้าไฟล์เพลยลิสต์เพิ่ม",  self.append_playlist, icon="➕", color="#4a90d9", tooltip="Append another playlist to current list").pack(side="left", padx=4)
        tk.Frame(btn_row, width=1, background=C["border"]).pack(side="left", padx=8, fill="y")
        make_btn(btn_row, "เช็คลิ้งค์",      self.start_check, icon="🔍", color="#f9ca24", tooltip="Check all unchecked links").pack(side="left", padx=4)
        make_btn(btn_row, "เช็คลิ้งค์ที่เลือก",   self.check_selected, icon="🎯", color="#e67e22", tooltip="Check only selected links").pack(side="left", padx=4)
        self._stop_btn = make_btn(btn_row, "หยุด เช็คลิ้งค์", self.stop_check, icon="⏹", color=C["bad"], tooltip="Stop link checking")
        self._stop_btn.pack(side="left", padx=4)
        self._stop_btn.pack_forget()
        tk.Frame(btn_row, width=1, background=C["border"]).pack(side="left", padx=8, fill="y")
        make_btn(btn_row, "ลบลิ้งค์เสีย", self.remove_bad, icon="🗑", color=C["bad"], tooltip="Delete all links marked as BAD").pack(side="left", padx=4)
        make_btn(btn_row, "ล้างรายการทั้งหมด", self.clear_all, icon="💥", color="#57606f", tooltip="Clear entire playlist").pack(side="left", padx=4)
        make_btn(btn_row, "ส่งออกลิ้งค์ที่เลือก", self.export_selected, icon="💾", color=C["accent3"], tooltip="Export selected channels").pack(side="right", padx=4)
        make_btn(btn_row, "ส่งออกลิ้งค์ทั้งหมด",      self.export_all, icon="📤", color=C["accent"], tooltip="Export all visible channels").pack(side="right", padx=4)
        separator(self, pad=0)
        fbar = tk.Frame(self, background=C["bg2"], pady=6)
        fbar.pack(fill="x")
        fbar_in = tk.Frame(fbar, background=C["bg2"])
        fbar_in.pack(fill="x", padx=12)
        tk.Label(fbar_in, text="🔎", background=C["bg2"], foreground=C["text_dim"], font=("TH Sarabun New", 11)).pack(side="left")
        se = tk.Entry(fbar_in, textvariable=self._search_var, background=C["card"], foreground=C["text"], insertbackground=C["accent"], font=FONT_NORMAL, relief="flat", width=30)
        ...
        tk.Label(fbar_in, text="Group:", background=C["bg2"], foreground=C["text_dim"], font=FONT_SMALL).pack(side="left")
        self._group_cb = ttk.Combobox(fbar_in, textvariable=self._group_var, state="readonly", width=22, font=FONT_SMALL)
        self._group_cb["values"] = ["All Groups"]
        self._group_cb.pack(side="left", padx=(4, 12), ipady=3)
        tk.Label(fbar_in, text="Status:", background=C["bg2"], foreground=C["text_dim"], font=FONT_SMALL).pack(side="left")
        scb = ttk.Combobox(fbar_in, textvariable=self._status_var, state="readonly", width=16, font=FONT_SMALL)
        scb["values"] = ["All Status", "good", "bad", "unchecked", "checking"]
        scb.pack(side="left", padx=(4, 12), ipady=3)
        tk.Checkbutton(fbar_in, text="Auto-play on select", variable=self._auto_play, background=C["bg2"], foreground=C["text_dim"], selectcolor=C["card"], activebackground=C["bg2"], font=FONT_SMALL).pack(side="left", padx=8)
        self._filter_label = tk.Label(fbar_in, text="Showing 0 / 0", background=C["bg2"], foreground=C["text_muted"], font=FONT_SMALL)
        self._filter_label.pack(side="right", padx=8)
        tk.Label(fbar_in, text="Ctrl+A: Select All  |  Del: Remove  |  Enter/DblClick: Play", background=C["bg2"], foreground=C["text_muted"], font=FONT_SMALL).pack(side="right", padx=12)
        separator(self, pad=0)
        main_pane = tk.PanedWindow(self, orient="horizontal", background=C["bg"], sashwidth=5, sashrelief="flat")
        main_pane.pack(fill="both", expand=True)
        table_frame = tk.Frame(main_pane, background=C["bg"])
        self._build_table(table_frame)
        main_pane.add(table_frame, minsize=520)
        right_pane = tk.PanedWindow(main_pane, orient="vertical", background=C["bg"], sashwidth=5, sashrelief="flat")
        ...
        self._statusbar = tk.Label(self, text="Ready  —  Import a playlist to get started", background=C["bg2"], foreground=C["text_dim"], font=FONT_SMALL, anchor="w", pady=5)
        self._statusbar.pack(fill="x", side="bottom", padx=12)
        self._progress_frame = tk.Frame(self, background=C["bg2"])
        self._progress_bar   = tk.Canvas(self._progress_frame, height=3, background=C["bg2"], highlightthickness=0)
    ...
    def _build_player_panel(self, parent):
        ...
        tk.Label(hdr, text="▶  Preview", font=FONT_HEADER, background=C["panel"], foreground=C["accent"]).pack(side="left")
        self._now_playing_lbl = tk.Label(hdr, text="No channel selected", font=FONT_SMALL, background=C["panel"], foreground=C["text_muted"])
        ...
        self._video_frame = tk.Frame(video_border, background=C["player_bg"], cursor="hand2")
        self._video_frame.pack(fill="both", expand=True)
        self._video_frame.bind("<Button-1>", lambda e: self._toggle_pause())
        self._video_hint = tk.Label(self._video_frame, text="🎬\nSelect a channel and press Play\nor enable Auto-play", background=C["player_bg"], foreground=C["text_muted"], font=FONT_NORMAL, justify="center")
        ...
        self._play_btn = make_icon_btn(ctrl, "▶", self.play_vlc, bg=C["good"],  fg="white")
        self._play_btn.pack(side="left", padx=2)
        make_icon_btn(ctrl, "⏸", self._toggle_pause, bg=C["accent"], fg="white").pack(side="left", padx=2)
        make_icon_btn(ctrl, "⏹", self._stop_player, bg="#57606f", fg="white").pack(side="left", padx=2)
        tk.Frame(ctrl, width=12, background=C["panel"]).pack(side="left")
        tk.Label(ctrl, text="🔊", background=C["panel"], foreground=C["text_dim"], font=("TH Sarabun New", 10)).pack(side="left")
        vol_slider = tk.Scale(ctrl, variable=self._vol_var, from_=0, to=200, orient="horizontal", command=self._on_volume_change, length=110, showvalue=True, background=C["panel"], foreground=C["text_dim"], troughcolor=C["card"], activebackground=C["accent"], highlightthickness=0, bd=0, font=FONT_LABEL, sliderrelief="flat", sliderlength=14)
        vol_slider.pack(side="left", padx=4)
        self._player_state_lbl = tk.Label(ctrl, text="●  Stopped", background=C["panel"], foreground=C["text_muted"], font=FONT_SMALL)
        self._player_state_lbl.pack(side="right", padx=8)

    def _build_info_panel(self, parent):
        pane = tk.PanedWindow(parent, orient="horizontal", background=C["panel"], sashwidth=4, sashrelief="flat")
        pane.pack(fill="both", expand=True)
        det = tk.Frame(pane, background=C["panel"])
        pane.add(det, minsize=180)
        tk.Label(det, text="Channel Info", font=FONT_HEADER, background=C["panel"], foreground=C["accent"]).pack(pady=(10, 4), padx=12, anchor="w")
        separator(det)
        self._detail_vars = {}
        fields = [("name","Name"), ("group","Group"), ("tvg_id","TVG-ID"), ("status","Status"), ("latency_ms","Latency"), ("url","URL"), ("tvg_logo","Logo URL")]
        for key, label in fields:
            row = tk.Frame(det, background=C["panel"])
            row.pack(fill="x", padx=12, pady=2)
            tk.Label(row, text=label + ":", font=FONT_SMALL, background=C["panel"], foreground=C["text_muted"], width=9, anchor="w").pack(side="left")
            var = tk.StringVar(value="—")
            tk.Entry(row, textvariable=var, background=C["card"], foreground=C["text"], font=FONT_SMALL, relief="flat", state="readonly", readonlybackground=C["card"]).pack(side="left", fill="x", expand=True, ipady=3, ipadx=4)
            self._detail_vars[key] = var
        separator(det)
        for text, cmd, color in [
            ("📋  คัดลอกลิ้งค์",           self._copy_url,      C["accent"]),
            ("✏️  แก้ไขชื่อช่อง",          self._edit_name,     "#4a90d9"),
            ("🗑  ลบรายการ",       self.remove_selected, C["bad"]),
        ]:
        ...
        tk.Label(log_fr, text="Activity Log", font=FONT_HEADER, background=C["panel"], foreground=C["accent"]).pack(pady=(10, 4), padx=12, anchor="w")
        separator(log_fr)
        ...
        self._log = tk.Text(lf2, background=C["card"], foreground=C["text_dim"], font=FONT_MONO, relief="flat", state="disabled", wrap="word", yscrollcommand=log_vsb.set)
    ...
    def _build_table(self, parent):
        ...
        col_conf = {"#": (44, 44, "#"), "status": (72, 72, "Status"), "name": (240, 240, "Channel Name"), "group": (140, 140, "Group"), "url": (340, 340, "URL"), "latency": (82, 82, "Latency")}
    ...
    def _stat_badge(self, parent, label, value, color):
        ...
        val = tk.Label(fr, text=value, font=("TH Sarabun New", 15, "bold"), background=C["bg3"], foreground=color)
        val.pack()
        tk.Label(fr, text=label, font=FONT_LABEL, background=C["bg3"], foreground=C["text_muted"]).pack()
        return val

    def _style_treeview(self):
        style = ttk.Style(self)
        style.theme_use("clam")
        style.configure("Treeview", background=C["bg3"], foreground=C["text"], fieldbackground=C["bg3"], rowheight=28, font=FONT_NORMAL, borderwidth=0)
        style.configure("Treeview.Heading", background=C["card"], foreground=C["text_dim"], font=FONT_SMALL, relief="flat", borderwidth=0)
        ...
        self.tree.tag_configure("playing",   background="#1a3320", foreground=C["good"])

    def _style_comboboxes(self):
        style = ttk.Style(self)
        style.configure("TCombobox", fieldbackground=C["card"], background=C["card"], foreground=C["text"], selectbackground=C["select_bg"], selectforeground=C["select_fg"], arrowcolor=C["accent"])
        style.map("TCombobox", fieldbackground=[("readonly", C["card"])])
        style.configure("TScrollbar", background=C["scrollbar"], troughcolor=C["bg2"], borderwidth=0, arrowcolor=C["accent"])

    def _make_context_menu(self):
        self._ctx = tk.Menu(self, tearoff=0, background=C["card"], foreground=C["text"], activebackground=C["select_bg"], activeforeground=C["select_fg"], font=FONT_SMALL)
    ...
    def play_vlc(self, channel: dict = None):
        if channel is None:
            ...
            if not sel: messagebox.showinfo("Info", "Select a channel first."); return
            channel = sel[0]
        if not VLC_AVAILABLE or not self._vlc or not self._vlc._ready:
            self._open_external_vlc(channel["url"]); return
    ...
    def _toggle_pause(self):
        if not self._vlc or not self._vlc._ready: return
        if self._vlc.is_playing():
            self._vlc.pause_resume(); self._set_player_state("paused"); self.log("⏸ Paused")
        else:
            self._vlc.pause_resume(); self._set_player_state("playing"); self.log("▶ Resumed")
    ...
    def _set_player_state(self, state: str):
        icons = {"playing": ("●  Playing", C["good"]), "paused": ("⏸  Paused", C["warning"]), "stopped": ("●  Stopped", C["text_muted"]), "error": ("✗  Error", C["bad"])}
    ...
    def _on_volume_change(self, val):
        if self._vlc: self._vlc.set_volume(int(val))

    def _open_external_vlc(self, url: str):
        vlc_paths = [r"C:\Program Files\VideoLAN\VLC\vlc.exe", r"C:\Program Files (x86)\VideoLAN\VLC\vlc.exe", "/usr/bin/vlc", "/usr/local/bin/vlc", "/Applications/VLC.app/Contents/MacOS/VLC", "vlc"]
        for p in vlc_paths:
            if Path(p).exists() or p == "vlc":
                try: subprocess.Popen([p, url]); return
                except Exception: continue
        messagebox.showerror("VLC Not Found", "VLC not found.\nhttps://www.videolan.org/")

    def _highlight_playing_row(self):
        for item in self.tree.get_children():
            tags = list(self.tree.item(item, "tags"))
            if "playing" in tags: tags.remove("playing"); self.tree.item(item, tags=tags)
    ...
    def _on_select(self, _=None):
        ...
        self._detail_vars["latency_ms"].set(f"{ch['latency_ms']} ms" if ch["latency_ms"] else "—")
        self._detail_vars["url"]       .set(ch["url"])
        self._detail_vars["tvg_logo"]  .set(ch["tvg_logo"] or "—")
        if self._auto_play.get(): self.play_vlc(ch)

    def _context_menu(self, event):
        item = self.tree.identify_row(event.y)
        if item and item not in self.tree.selection(): self.tree.selection_set(item)
        if self.tree.selection(): self._ctx.post(event.x_root, event.y_root)
    ...
    def _refresh_table(self):
        ...
        for i, ch in enumerate(self.filtered):
            ...
            tags   = [tag]
            if ch is self._now_playing: tags.append("playing")
            self.tree.insert("", "end", iid=str(i), values=(i+1, icon, ch["name"], ch["group"], ch["url"], lat), tags=tags)
        self._update_stats()
    ...
    def _hide_progress(self): self._progress_frame.pack_forget()
    ...
    def _sort_by(self, col: str):
        ...
        key_map = {"#": lambda c: self.channels.index(c) if c in self.channels else 0, "status": lambda c: c["status"], "name": lambda c: c["name"].lower(), "group": lambda c: c["group"].lower(), "url": lambda c: c["url"], "latency": lambda c: c["latency_ms"] or 99999}
        self.filtered.sort(key=key_map.get(col, lambda c: c["name"].lower()), reverse=self.sort_rev)
        self._refresh_table()
    ...
    def set_status(self, msg: str): self._statusbar.config(text=msg)

    def _open_file_dialog(self):
        return filedialog.askopenfilename(title="Select Playlist File", filetypes=[("Playlist files", "*.m3u *.m3u8 *.w3u"), ("M3U files", "*.m3u *.m3u8"), ("W3U files", "*.w3u"), ("All files", "*.*")])
    ...
    def import_playlist(self):
        ...
        try:
            ...
            if not ch: messagebox.showwarning("Empty", "No channels found."); return
            self.channels = ch
    ...
    def append_playlist(self):
        ...
        try:
            ...
            if not new_ch: messagebox.showwarning("Empty", "No channels found."); return
    ...
    def start_check(self):
        if self._checking: messagebox.showinfo("Info", "Check already in progress."); return
        targets = [c for c in self.channels if c["status"] != "good"]
        if not targets: messagebox.showinfo("Info", "All links already checked / good."); return
        self._run_check(targets)

    def check_selected(self):
        targets = self._get_selected_channels()
        if not targets: messagebox.showinfo("Info", "No channels selected."); return
        self._run_check(targets)
        ...
        def worker():
            ...
            for i, ch in enumerate(targets):
                if self._check_stop.is_set():
                    self.after(0, lambda: self.log("Link check stopped by user.")); break
                ch["status"] = "checking"
                self.after(0, self.apply_filters)
                self.after(0, lambda m=f"Checking {i+1}/{total}: {ch['name'][:50]}": self.set_status(m))
    ...
    def stop_check(self): self._check_stop.set()

    def remove_bad(self):
        bad = [c for c in self.channels if c["status"] == "bad"]
        if not bad: messagebox.showinfo("Info", "No bad links to remove."); return
    ...
    def _export_channels(self, channels: list):
        if not channels: messagebox.showinfo("Info", "Nothing to export."); return
        fmt_var = tk.StringVar(value="m3u")
        dlg = tk.Toplevel(self); dlg.title("Export Options"); dlg.configure(background=C["bg"]); dlg.geometry("340x200"); dlg.resizable(False, False); dlg.grab_set()
        tk.Label(dlg, text=f"Export {len(channels)} channels", font=FONT_HEADER, background=C["bg"], foreground=C["text"]).pack(pady=(16, 4))
        tk.Label(dlg, text="Choose output format:", font=FONT_SMALL, background=C["bg"], foreground=C["text_dim"]).pack()
        row = tk.Frame(dlg, background=C["bg"]); row.pack(pady=8)
        for fmt, lbl in [("m3u", ".M3U — Standard playlist"), ("w3u", ".W3U — JSON playlist")]:
            tk.Radiobutton(row, text=lbl, variable=fmt_var, value=fmt, background=C["bg"], foreground=C["text"], selectcolor=C["card"], activebackground=C["bg"], font=FONT_SMALL).pack(anchor="w", padx=20, pady=2)
        def do_export():
            fmt = fmt_var.get()
            path = filedialog.asksaveasfilename(defaultextension=f".{fmt}", filetypes=[(f"{fmt.upper()} files", f"*.{fmt}"), ("All files", "*.*")], initialfile=f"playlist.{fmt}")
            if not path: dlg.destroy(); return
            try:
    ...
    def export_selected(self): self._export_channels(self._get_selected_channels())
    def export_all(self): self._export_channels(self.filtered)

    def _copy_url(self):
        sel = self._get_selected_channels()
        if not sel: return
        self.clipboard_clear(); self.clipboard_append(sel[0]["url"])
    ...
    def _edit_name(self):
        ...
        if not sel: return
        ch = sel[0]
        dlg = tk.Toplevel(self); dlg.title("Edit Channel Name"); dlg.configure(background=C["bg"]); dlg.geometry("420x130"); dlg.resizable(False, False); dlg.grab_set()
        tk.Label(dlg, text="Channel Name:", font=FONT_SMALL, background=C["bg"], foreground=C["text_dim"]).pack(pady=(16, 4))
        var = tk.StringVar(value=ch["name"])
        entry = tk.Entry(dlg, textvariable=var, font=FONT_NORMAL, background=C["card"], foreground=C["text"], insertbackground=C["accent"], relief="flat", width=48)
        entry.pack(padx=20, ipady=6); entry.focus()
        def save(): ch["name"] = var.get().strip() or ch["name"]; self.apply_filters(); self.log(f"Renamed → {ch['name']}"); dlg.destroy()
